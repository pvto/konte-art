...
public class UtBuilder implements
    ...
    public UtBuilder addShape(Matrix4... points) {
        if (shapes == null) {
            shapes = new ArrayList<ArrayList<Matrix4>>();
        }
        ArrayList<Matrix4> shape = new ArrayList<Matrix4>();
        for (Matrix4 m : points) {
            shape.add(m);
        }
        if (shape.size() > 0) {
            shapes.add(shape);
    ...
    public UtBuilder addControlPoints(Matrix4[][] points) {
        if (controlPoints == null) {
            controlPoints = new ArrayList<ArrayList<Matrix4[]>>();
        }
        ArrayList<Matrix4[]> cps = new ArrayList<Matrix4[]>();
        // add points in supplied order, pad each pair to two if one or zero supplied points
        for (Matrix4[] mm : points) { // shapes
        ...
        for (int i = 0; i < shapes.get(controlPoints.size()).size() - cps.size(); i++) {
            ArrayList<Matrix4[]> cps2 = new ArrayList<Matrix4[]>();
    ...
    private static ArrayList<Untransformable> instances = new ArrayList<Untransformable>();
